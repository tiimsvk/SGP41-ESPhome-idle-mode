...
from esphome.components import i2c, sensirion_common, sensor, switch
...
DEPENDENCIES = ["i2c", "switch"]  # <- doplnené switch
AUTO_LOAD = ["sensirion_common", "switch"]  # <- aby sa natiahli zdroje a hlavičky
...
SGP4xHeaterSwitch = sgp4x_ns.class_("SGP4xHeaterSwitch", switch.Switch)

CONF_HUMIDITY_SOURCE = "humidity_source"
CONF_HEATER_ENABLE = "heater_enable"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_HEATER_ENABLE, default=True): cv.boolean,
            cv.Optional(CONF_SWITCH_POWER): switch.switch_schema(
                SGP4xHeaterSwitch,
                icon=ICON_RADIATOR,
            ),
        }
...
async def to_code(config):
    ...
    if CONF_VOC_BASELINE in config:
        cg.add(var.set_voc_baseline(config[CONF_VOC_BASELINE]))

    cg.add(var.set_heater_enabled(config[CONF_HEATER_ENABLE]))

    if CONF_SWITCH_POWER in config:
        sw_conf = config[CONF_SWITCH_POWER]
